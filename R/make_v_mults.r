...
inteff <- colSums(mprod*impact*as.matrix(lookmult[sector]))

# F. Calculate indirect impacts from EC
empcomp <- colSums(costf[costf$Type %in% c("Captain", "Crew"),
    !names(costf) %in% c("ID", "Type", "Cost")])*impact*
    ecpi[ecpi$Type == paste0("EmpComp", type), c(sector)]

# G. Calculate indirect impacts from PI
propinc <- colSums(costf[costf$Type %in% c("Purchases of permits",
...
looktaxes <- colSums(costf[costf$Type %in% c("Landings Taxes"),
    !names(costf) %in% c("ID", "Type", "Cost")])*impact*
...
sumdirect <- inteff + empcomp + propinc + looktaxes +
    colSums(costf[costf$Type %in% c("Captain", "Crew", "Purchases of permits",
        "Leasing of permits", "Proprietary income"),
    !names(costf) %in% c("ID", "Type", "Cost")])*impact

dirinc <- colSums(costf[costf$Type %in% c("Captain", "Crew", "Purchases of permits",
        "Leasing of permits", "Proprietary income"),
    !names(costf) %in% c("ID", "Type", "Cost")])*impact
...
sumdirect <- inteff + empcomp + propinc + looktaxes +
    as.numeric(impact/(costf[costf$Type %in% c("Output per Employee"),
    !names(costf) %in% c("ID", "Type", "Cost")]))

diremp <- as.numeric(impact/(costf[costf$Type %in% c("Output per Employee"),
    !names(costf) %in% c("ID", "Type", "Cost")]))
